...
class WordSegment(CRFTagger):
    """
    python-CRFsuite로 구현한 띄어쓰기 모델입니다.

    >>> from konlp.tokenize import WordSegment
    >>> sp = WordSegment()

    >>> train_data = [[['한','B'], ['용','I'], ['운','I'], ['씨','B'], ['는','I'], ['행','B'], ['복','I'], ['했','I'], ['다','I']]
    ... [['나','B'], ['에','I'], ['게','I'], ['적','B'], ['은','I'], ['없','B'],  ['다','I']]]

    >>> sp.train(train_data,'Spacing.model')
    >>> sp.tag("한용운씨는 사람이다.")
    한용운 씨는 사람이다.
    ...
    def tag(self, sent):
        ...
        Args:
            sent(str): 문자열을 인자로 받습니다.

        Returns:
            spacing_sent(str): 띄어쓰기가 완료된 문자열을 반환합니다.
        ...
        sentence = [(s, t) for s, t in zip(syllables, tags) if not s == self.SP]
        predicted_tags = self._tagger.tag(self._get_features(sentence))
        index = 0
        for _, l, i in self._squeeze_entities(index, predicted_tags):
            result.append("".join(i[0] for i in sentence[i:i + l]))
