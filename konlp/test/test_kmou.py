...
def test_WordSegment():
    WS = WordSegment()
    WS.set_model_file("file\SPACE.crf.model")
    assert WS.tag("아버지가방에들어간다") == "아버지가 방에 들어간다."


def test_VXP_ChunkTagger(input_string):
    vxp = VXP_ChunkTagger()
    vxp.set_chunk_rule("file\ChunkingRule_VXP.txt")
...
def test_JSP_ChunkTagger(input_string):
    jsp = JSP_ChunkTagger()
    jsp.set_chunk_rule("file\ChunkingRule_JSP.txt")
...
def test_NERTagger():
    NERT = NERTagger()
    NERT.set_model_file(r"file\NER.crfsuite")
